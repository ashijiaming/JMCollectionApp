 ...
 */
public class MediaRecorderNative extends MediaRecorderBase implements MediaRecorder.OnErrorListener, FFmpegJni.FFmpegStateListener {

    public MediaRecorderNative() {
        FFmpegJni.registFFmpegStateListener(this);
    }
    ...
    public MediaObject.MediaPart startRecord() {
        ...
        if (mCameraId == Camera.CameraInfo.CAMERA_FACING_BACK) {
            vCustomFormat= FFmpegJni.ROTATE_90_CROP_LT;
        } else {
            vCustomFormat= FFmpegJni.ROTATE_270_CROP_LT_MIRROR_LR;
        }

        FFmpegJni.prepareJXFFmpegEncoder( mMediaObject.getOutputDirectory(), mMediaObject.getBaseName(),vCustomFormat, mSupportedPreviewWidth, SMALL_VIDEO_HEIGHT, SMALL_VIDEO_WIDTH, SMALL_VIDEO_HEIGHT, mFrameRate, mVideoBitrate);
    ...
    public void onPreviewFrame(byte[] data, Camera camera) {
        if (mRecording) {
            FFmpegJni.encodeFrame2H264(data);
...
//        UtilityAdapter.RenderOutputSettings(SMALL_VIDEO_WIDTH, SMALL_VIDEO_HEIGHT, mFrameRate, UtilityAdapter.OUTPUTFORMAT_YUV | UtilityAdapter.OUTPUTFORMAT_MASK_MP4/*| UtilityAdapter.OUTPUTFORMAT_MASK_HARDWARE_ACC*/);
    ...
    public void receiveAudioData(byte[] sampleBuffer, int len) {
        if (mRecording && len > 0) {
            FFmpegJni.encodeFrame2AAC(sampleBuffer);
        }
    }
    ...
    public void activityStop(){
        FFmpegJni.unRegistFFmpegStateListener(this);
    }
}
