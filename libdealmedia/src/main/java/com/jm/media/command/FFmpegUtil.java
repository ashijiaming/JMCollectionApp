...
/**
 * ffmpeg工具：拼接命令行处理音视频
 * Created by ShiJiaMing on 2019/12/23.
 */
...
public class FFmpegUtil {
     ...
     * @return 合并后的文件
     */
    public static String[] concatAudioWithList(String srcList, String targetFile) {
        deleteTargetFile(targetFile);
        String concatAudioCmd = "-i concat:%s -acodec copy %s";
        concatAudioCmd = String.format(concatAudioCmd, srcList, targetFile);
        return concatAudioCmd.split(" ");//以空格分割为字符串数组
    }
     ...
     */
    @SuppressLint("DefaultLocale")
    public static String[] mediaMuxWithDuration(String videoFile, String audioFile, int duration, String muxFile) {
        //-t:时长  如果忽略音视频时长，则把"-t %d"去掉
        String mixAudioCmd = "-i %s -i %s -t %d -vcodec copy %s";
        mixAudioCmd = String.format(mixAudioCmd, videoFile, audioFile, muxFile);
        return mixAudioCmd.split(" ");//以空格分割为字符串数组
    }
     ...
     */
    @SuppressLint("DefaultLocale")
    public static String[] mediaMuxPack(String videoFile, String audioFile, String muxFile) {
        String mixAudioCmd = "-i %s -i %s -c:v copy -c:a mp3 -strict experimental %s";
        mixAudioCmd = String.format(mixAudioCmd, videoFile, audioFile, muxFile);
        return mixAudioCmd.split(" ");//以空格分割为字符串数组
    }
     ...
     * @return
     */
    public static String[] audioAddVolume(String srcFile,String maxFile){
        deleteTargetFile(maxFile);
        String volumeAudio="-i %s -vol %s %s";
        volumeAudio=String.format(volumeAudio,srcFile,"1500",maxFile);
        return  volumeAudio.split(" ");
    }
    ...
    @SuppressLint("DefaultLocale")
    public static String[] audioMuxPack(String srcFile, String audioFile, String muxFile) {
        deleteTargetFile(muxFile);
        String mixAudioCmd = "-i %s -i %s -filter_complex amerge -ac 2 -c:a libmp3lame -q:a 4 %s";
        mixAudioCmd = String.format(mixAudioCmd, srcFile, audioFile, muxFile);
        return mixAudioCmd.split(" ");//以空格分割为字符串数组
    }
    ...
    /**
     * 音频替换 将视频中的音频直接替换（比较耗时）
     * @param srcFile
     * @param audio
     * @param targetFile
     * @return
     */
    public static String[] replaceAudio(String srcFile, String audio, String targetFile) {
        deleteTargetFile(targetFile);
        String mixAudioCmd = "-i %s -i %s -c:v copy -c:a aac -strict experimental -map 0:v:0 -map 1:a:0 %s";
        mixAudioCmd = String.format(mixAudioCmd, srcFile, audio, targetFile);
        return mixAudioCmd.split(" ");//以空格分割为字符串数组
    }
...
//        String reverseVideo = "-i %s -filter_complex [0:v]reverse[v];[0:a]areverse[a] -map [v] -map [a] %s";
    ...
    private static void deleteTargetFile(String path) {
