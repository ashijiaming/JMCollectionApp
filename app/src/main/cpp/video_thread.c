...
void init_input_format_ctx(struct Player *player,const char* input_str){
    //注册组件
    av_register_all();
    //封装格式上下文
    ...
    //打开输入视频文件
    if(avformat_open_input(&format_ctx,input_str,NULL,NULL)!=0){
        LOGI("%s","打开输入文件失败");
        return;
    }
    //获取视频信息
    ...
    //视频解码，需要找到视频对应的AVStream所在format_ctx->stream的索引位置
    //获取音频和视频流的索引用
    int i;
...
void decode_video(struct Player *player,AVPacket *packet){
    ...
    if(got_frame){
        ...
        avpicture_fill((AVPicture *)rgb_frame,outBuffer.bits,
...
Java_jm_com_collection_ffmpeg_jni_ThreadVideo_decodeVideo(JNIEnv *env, jobject instance,
